  ...
  async function main() {
    ...
    for (let vault of allVault ){
      if(vault.underlyingTokenMint.toString() == NATIVE_MINT.toString()){
        ...
        let depositTx = await katana.coverCall.deposit(vault,walletPublicKey, new BN(1),connection)
        tx.add(depositTx);
        let withdrawTx = await katana.coverCall.instantWithdraw(vault,walletPublicKey,new BN(1))
        tx.add(withdrawTx)
        let initiateWithdraw = await katana.coverCall.initiateWithdraw(vault,walletPublicKey,new BN(0))
        tx.add(initiateWithdraw);
        let completeWithdraw = await katana.coverCall.completeWithdraw(vault,walletPublicKey)
        //tx.add(completeWithdraw)
        let claimshare = await katana.coverCall.claimShares(vault,walletPublicKey);
        //tx.add(claimshare);
    ...
    let allUserVault = await katana.coverCall.getAllUserVault(connection,walletPublicKey);
