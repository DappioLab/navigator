...
import { getTokenAccountAmount } from "../src/util";
...
export async function getAllSwap(connection: Connection) {
  ...
  for (let account of allSaberAccount) {
    ...
    let saberAccountInfo = await layout.parseSwapInfoData(
...
export async function getSwap(connection: Connection, swap: PublicKey) {
  ...
  const saberAccountInfo = await layout.parseSwapInfoData(
    saberAccount.data,
    swap
  );
