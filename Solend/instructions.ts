...
export async function createObligationAccountIx(
  wallet: PublicKey,
  payer?: PublicKey
) {
  ...
  let createAccountFromSeedIx = await SystemProgram.createAccountWithSeed({
    fromPubkey: payer,
    seed: seed,
    space: 1300,
  ...
  let keys = [
    {
      pubkey: await getObligationPublicKey(wallet),
      isSigner: false,
      isWritable: true,
    },
