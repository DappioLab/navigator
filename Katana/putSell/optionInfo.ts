...
export async function getAllOptionPrams(connection:Connection){
    ...
    const allOptionPramAccount = await connection.getProgramAccounts(KATANA_PROGRAM_ID, config);
    let allOptionPram :OptionParameters[] = []
    for(let accountInfo of allOptionPramAccount){
        allOptionPram.push( await parseOptionParameters(accountInfo.account.data,accountInfo.pubkey))
    }
