...
infos = class InstanceFriktion {
  static async getAllVaults(
    ...
    const allVaultAccount = paginate(
      await connection.getProgramAccounts(VOLT_PROGRAM_ID, config),
      page
    );
    return allVaultAccount.map((account) => {
      ...
      for (let round = 1; round < Number(vault.roundNumber) + 1; round++) {
        ...
        if (epochInfo) {
          ...
          let emptyEpochInfo = EPOCH_INFO_LAYOUT.decode(
            Buffer.alloc(10000)
          ) as types.EpochInfo;
          vault.epochInfos.push({ ...emptyEpochInfo, epochId: epochId });
        }
      }
      vault.extraData = extraInfos.get(
        vaultWrapper.getExtraVoltDataAddress().toString()
      ) as types.ExtraVaultInfo;
      vault.extraData.entropyMetadataInfo = entropyMetaDataSet.get(
        vault.extraData.entropyMetadata.toString()
      );
  ...
  static async getAllVaultWrappers(
    connection: Connection,
    page?: PageConfig
  ): Promise<IVaultInfoWrapper[]> {
    return (await this.getAllVaults(connection, page)).map(
      (vault) => new VaultInfoWrapper(vault)
    );
  }
  ...
  static parseVault(data: Buffer, vaultId: PublicKey): types.VaultInfo {
    const vault = VOLT_VAULT_LAYOUT.decode(
      Buffer.from(data)
    ) as types.VaultInfo;
  ...
  static async getAllDepositors(
    ...
    let allPendingDepositMeta = (
      await utils.getMultipleAccounts(connection, allPendingDepositKeys)
    ).filter((info) => Boolean(info.account));
  ...
  static getDepositorId(
    ...
    return PublicKey.findProgramAddressSync(
      [
        vaultId.toBuffer(),
        userKey.toBuffer(),
        new Uint8Array(Buffer.from("pendingDeposit", "utf-8")),
      ],
      programId
    )[0];
  }
  static parseDepositor(
    data: Buffer,
    depositorId: PublicKey,
    userKey?: PublicKey
  ): types.DepositorInfo {
  ...
  static getWithdrawerId(
    ...
    return PublicKey.findProgramAddressSync(
      [
        vaultId.toBuffer(),
        userKey.toBuffer(),
        new Uint8Array(Buffer.from("pendingWithdrawal", "utf-8")),
      ],
      programId
    )[0];
  }
  ...
  static parseWithdrawer(
    data: Buffer,
    withdrawerId: PublicKey,
    userKey?: PublicKey
  ): types.withdrawerInfo {
    if (!userKey) {
      throw new Error("User key not provided");
    }
    return {
      ...(USER_PENDING_LAYOUT.decode(
        Buffer.from(data)
      ) as types.withdrawerInfo),
      withdrawerId: withdrawerId,
      userKey: userKey,
    };
  }
  static async getAllWithdrawers(
    connection: Connection,
    userKey: PublicKey
  ): Promise<types.withdrawerInfo[]> {
    let vaults = await this.getAllVaults(connection);
    ...
    let allPendingWithdrawalMeta = (
      await utils.getMultipleAccounts(connection, allPendingWithdrawalKeys)
    ).filter((info) => Boolean(info.account));
  ...
  private static async _getAllRoundSet(connection: Connection) {
    ...
    let allRoundKeys = await connection.getProgramAccounts(
      VOLT_PROGRAM_ID,
      config
    );
    return new Map(
  ...
  private static async _getAllExtraDataSet(connection: Connection) {
    ...
    let config: GetProgramAccountsConfig = { filters: [sizeFilter] };
    let allExtraDataKeys = await connection.getProgramAccounts(
      VOLT_PROGRAM_ID,
      config
    );
    return new Map(
  ...
  private static async _getSnapshotSet() {
    ...
    const snapshotData = (await (
      await axios.get(snapshotUrl)
    ).data.allMainnetVolts) as types.SnapshotInfo[];
    return new Map(
      snapshotData.map((key) => [key.voltVaultId.toString(), key])
    );
  }
};
export class VaultInfoWrapper {
  constructor(public readonly vaultInfo: types.VaultInfo) {}
  getExtraVoltDataAddress(
    voltProgramId: PublicKey = VOLT_PROGRAM_ID
  ): PublicKey {
    return PublicKey.findProgramAddressSync(
      [
        this.vaultInfo.vaultId.toBuffer(),
        new Uint8Array(Buffer.from("extraVoltData", "utf-8")),
      ],
      voltProgramId
    )[0];
  }
  getEntropyLendingAccountAddress(
    voltProgramId: PublicKey = VOLT_PROGRAM_ID
  ): PublicKey {
    return PublicKey.findProgramAddressSync(
      [
        this.vaultInfo.vaultId.toBuffer(),
        new Uint8Array(Buffer.from("entropyLendingAccount", "utf-8")),
      ],
      voltProgramId
    )[0];
  }
  getEntropyMetadataAddress(
    voltProgramId: PublicKey = VOLT_PROGRAM_ID
  ): PublicKey {
    return PublicKey.findProgramAddressSync(
      [
        this.vaultInfo.vaultId.toBuffer(),
        new Uint8Array(Buffer.from("entropyMetadata", "utf-8")),
      ],
      voltProgramId
    )[0];
  }
