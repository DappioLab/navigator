...
infos = class InstanceFriktion {
  static async getAllVaults(connection: Connection): Promise<types.VaultInfo[]> {
    let rounds = await getAllRoundMap(connection);
    ...
    return allVaultAccount.map((account) => {
      ...
      for (let round = 1; round < vault.roundNumber.toNumber()+1; round++) {
        let roundId = findRoundInfoAddress(vault.vaultId, new BN(round));
...
export async function findExtraVoltDataAddress(
  vaultId: PublicKey,
  voltProgramId: PublicKey = VOLT_PROGRAM_ID
): Promise<[PublicKey, number]> {
  return await PublicKey.findProgramAddress(
    [vaultId.toBuffer(), new Uint8Array(Buffer.from("extraVoltData", "utf-8"))],
    voltProgramId
  );
}

export async function findEntropyLendingAccountAddress(
  vaultId: PublicKey,
  voltProgramId: PublicKey = VOLT_PROGRAM_ID
): Promise<[PublicKey, number]> {
  return PublicKey.findProgramAddress(
    [vaultId.toBuffer(), new Uint8Array(Buffer.from("entropyLendingAccount", "utf-8"))],
    voltProgramId
  );
}

export function findRoundInfoAddress(
  vaultId: PublicKey,
  roundNumber: BN,
  voltProgramId: PublicKey = VOLT_PROGRAM_ID
): PublicKey {
  return PublicKey.findProgramAddressSync(
    [vaultId.toBuffer(), roundNumber.toBuffer("le", 8), new Uint8Array(Buffer.from("roundInfo", "utf-8"))],
    voltProgramId
  )[0];
}
export function findRoundVoltTokensAddress(
  vaultId: PublicKey,
  roundNumber: BN,
  voltProgramId: PublicKey = VOLT_PROGRAM_ID
): PublicKey {
...
export async function findRoundUnderlyingTokensAddress(
  voltKey: PublicKey,
  roundNumber: BN,
  voltProgramId: PublicKey = VOLT_PROGRAM_ID
): Promise<[PublicKey, number]> {
...
export async function getAllRoundMap(connection: Connection) {
  ...
  let allRoundInfos = allRoundKeys.map((meta) => {
    return { ...ROUND_LAYOUT.decode(meta.account.data), roundId: meta.pubkey } as types.RoundInfo;
  });
  return new Map(allRoundInfos.map((i) => [i.roundId.toString(), i]));
}
