...
import { LARIX_MARKET_ID_MAIN_POOL, LARIX_MINT, MARKET_PDA } from "./ids";
...
export async function createDepositTx(
  ...
  } else reserveTokenAddress = await findAssociatedTokenAddress(wallet, reserveInfoWrapper.reserveTokenMint());
  if (supplyTokenAddress) {
    supplyTokenAddress = supplyTokenAddress as PublicKey;
  } else supplyTokenAddress = await findAssociatedTokenAddress(wallet, reserveInfoWrapper.supplyTokenMint());

  let createAtaIx = await createATAWithoutCheckIx(wallet, reserveInfoWrapper.reserveTokenMint());
  tx.add(createAtaIx);
  ...
  if (reserveInfoWrapper.supplyTokenMint().equals(NATIVE_MINT)) {
    ...
    cleanupTx.add(createCloseAccountInstruction(supplyTokenAddress, wallet, wallet, []));
    tx.add(wrapIx);
  }
  ...
  tx.add(miningIx);
  cleanupTx.add(createCloseAccountInstruction(reserveTokenAddress, wallet, wallet, []));
  tx.add(cleanupTx);
  return tx;
}
...
export async function createWithdrawTx(
  ...
  } else withdrawTokenAddress = await findAssociatedTokenAddress(wallet, reserveInfoWrapper.supplyTokenMint());
  let createWithdrawTokenATAIx = await createATAWithoutCheckIx(wallet, reserveInfoWrapper.supplyTokenMint());
  tx.add(createWithdrawTokenATAIx);
  let createReserveATA = await createATAWithoutCheckIx(wallet, reserveInfoWrapper.reserveTokenMint());
  ...
  tx.add(withdrawIns);
  if (reserveInfoWrapper.supplyTokenMint().equals(NATIVE_MINT)) {
    cleanupTx.add(createCloseAccountInstruction(withdrawTokenAddress, wallet, wallet, []));
  }
  cleanupTx.add(createCloseAccountInstruction(reserveTokenAddress, wallet, wallet, []));
  tx.add(cleanupTx);
  return tx;
...
export async function claimReward(wallet: PublicKey, minerInfo: MinerInfo, connection: Connection) {
  let tx: Transaction = new Transaction();
  ...
  for (let index = 0; index < reserveAccounts.length; index++) {
    allReserve.push(parseReserveData(reserveAccounts[index]?.data, reservepub[index]));
  }
  for (let reserve of allReserve) {
    tx.add(
      refreshReserveInstruction(reserve.reserveId, reserve.liquidity.OraclePubkey, reserve.liquidity.larixOraclePubkey)
    );
  }
  ...
  tx.add(claimRewardIx(larixTokenAddress, minerInfo.farmerId, wallet, reservepub));
  return tx;
}
